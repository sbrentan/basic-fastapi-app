...
from funder.database.database import get_db, FunderDatabase
from funder.schemas.crud.transaction import TransactionCreate
...
async def create_transaction(new_transaction: TransactionCreate, db: FunderDatabase = Depends(get_db)):
...
async def read_transactions(db: FunderDatabase = Depends(get_db)):
...
async def read_transaction(transaction_id: int, db: FunderDatabase = Depends(get_db)):
...
async def update_transaction(transaction_id: int, new_transaction: TransactionCreate,
                             db: FunderDatabase = Depends(get_db)):
    transaction = Transaction(**new_transaction.model_dump())
...
async def delete_transaction(transaction_id: int, db: FunderDatabase = Depends(get_db)):
